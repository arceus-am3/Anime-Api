...
class AniplayExtractor {
  ...
  async fetchStaticJsUrl() {
    ...
    if (start === -1) {
      throw new Error("Static chunk path not found in HTML.");
    }
  ...
  async fetchEpisode(animeId, ep, host = "hika", type = "sub") {
    ...
    // Next.js RSC format → "1:{json}"
    const dataStr = text.split("1:")[1];
    if (!dataStr) {
      throw new Error("Invalid episode response format");
    }
